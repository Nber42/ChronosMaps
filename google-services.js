...
window.initGoogleServices = function () {
    console.log("initGoogleServices started. map ready?", !!window.map);
    if (!window.map) {
        console.warn('initGoogleServices: map not ready yet.');
        return;
    }
    ...
    try {
        ...
        if (window.map) {
            if (place && place.geometry && place.geometry.location) {
                ...
                window.map.panTo({ lat: chronosData.lat, lng: chronosData.lng });
            }
        }
    } catch (err) {
...
window.openStreetView = function () {
    ...
    const panorama = window.map.getStreetView();
