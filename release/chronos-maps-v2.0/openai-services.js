...
const GOOGLE_MAPS_API_KEY = 'AIzaSyBt3-_T5sn-4xua9SdE7D7ENrXly3R4qAo';
    ...
    showOpenAISetup: function () {
        ...
        const setupHTML = `
            <div class="api-setup-modal">
                ...
                <div style="background:#f0f9ff; padding:12px; border-radius:8px; margin-bottom:15px; font-size:13px; border-left:4px solid #3b82f6;">
                    <p style="margin:0;">No necesitas una clave para usar la app, ya que usará la <strong>Licencia del Sistema</strong> por defecto.</p>
                </div>

                <label style="display:block; font-weight:bold; margin-top:10px;">Tu API Key (Opcional):</label>
                <input type="password" 
                       id="openai-key-input" 
                       placeholder="sk-proj-..." 
                       value="${currentKey}"
                ...
                <div style="display:flex; gap:10px;">
                    <button onclick="OpenAIService.saveAndTestOpenAIKey()" style="flex:1; padding: 12px; margin-top: 8px;" class="btn-primary">
                      💾 Guardar Personal
                    </button>
                    <button onclick="OpenAIService.clearKey()" style="flex:1; padding: 12px; margin-top: 8px; background:#ef4444; color:white; border:none; border-radius:4px;">
                      🗑️ Borrar
                    </button>
                </div>
                
                <div class="setup-instructions" style="margin-top: 16px; font-size: 13px; background:#f5f5f5; padding:10px; border-radius:8px;">
                  <h4 style="margin-top:0;">📝 Nota:</h4>
                ...
                <button onclick="document.getElementById('generic-modal').classList.add('hidden')" style="margin-top: 12px; background: #ccc; border:none; padding:8px 16px; border-radius:4px; cursor:pointer; width:100%;">
                  Cerrar
                </button>
            </div>
        `;
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="modal-card" style="position:relative; max-width:500px; width:90%;">
                     <button class="close-btn" style="position:absolute; top:10px; right:15px; background:none; border:none; font-size:18px; cursor:pointer;" onclick="document.getElementById('generic-modal').classList.add('hidden')">✕</button>
    ...
    showPlacePanel: function (placeData, aiData) {
        ...
        panelContent.innerHTML = `
            <div class="panel-inner-content">
                <!-- HEADER NAVIGATION -->
                <div class="panel-header-actions" style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <button class="back-button" onclick="document.getElementById('side-panel').classList.remove('open')">← Cerrar</button>
                    <div class="rarity-badge ${rarity}" style="border-color: ${rarityColor}; color: ${rarityColor}; font-weight:bold; border:1px solid; padding:2px 8px; border-radius:4px;">
                        ${rarityStars} ${rarity.toUpperCase()}
                    </div>
                </div>

                <!-- IMAGEN PRINCIPAL -->
                <div class="place-hero-image" style="position:relative; margin-bottom:16px; border-radius:8px; overflow:hidden; height:200px;">
                    <img src="${mainPhoto}" alt="${placeData.name || placeData.displayAddress}" style="width:100%; height:100%; object-fit:cover;">
                    ${(isSpecificPlace && placeData.photos && placeData.photos.length > 1) ? `
                        <div class="photo-count" style="position:absolute; bottom:10px; right:10px; background:rgba(0,0,0,0.6); color:white; padding:4px 8px; border-radius:4px; font-size:12px;">
                ...
                <!-- TITLE & CATEGORY -->
                <div class="place-header" style="margin-bottom:12px;">
                    <h1 class="place-name" style="margin:0; font-size:24px; line-height:1.2;">${placeData.name || placeData.displayAddress}</h1>
                    ${isSpecificPlace ? `
                        <div class="place-category" style="margin-top:5px; color:#666; font-size:14px; display:flex; align-items:center; gap:5px;">
                            <span>${this.getCategoryIcon(placeData.category)}</span>
                            <span>${this.getCategoryName(placeData.category)}</span>
                            ${placeData.rating ? `
                                <span style="margin-left:10px;">⭐ ${placeData.rating} (${placeData.ratingsCount})</span>
                            ` : ''}
                        </div>
                    ` : ''}
                </div>
                ...
                <!-- ADDRESS (Secondary) -->
                <div class="place-address" style="display:flex; gap:8px; align-items:center; margin-bottom:16px; font-size:13px; color:#555;">
                ...
                ${placeData.editorialSummary ? `
                    <div class="editorial-summary" style="background:#f0f7ff; padding:12px; border-radius:8px; margin-bottom:16px; font-size:14px; border-left:3px solid #3b82f6;">
                        <p style="margin:0; font-style:italic;">"${placeData.editorialSummary}"</p>
                        <small style="display:block; margin-top:5px; color:#888;">Fuente: Google</small>
                    </div>
                ` : ''}

                <!-- MAIN HIGHLIGHT (AI) -->
                ${aiData.main_highlight ? `
                    <div class="main-highlight" style="background: #fff3cd; padding: 16px; border-radius: 8px; margin-bottom: 24px; border-left: 4px solid #FFD700;">
                        <span class="highlight-icon" style="font-size:20px; margin-right:5px;">⭐</span>
                        <span style="font-weight:600; color:#856404; font-size:16px;">${aiData.main_highlight}</span>
                    </div>
                ` : ''}

                 <!-- IMÁGENES ANTES/AHORA (Dynamic StreetView) -->
                 <div class="time-travel-visuals" style="margin-bottom: 24px;">
                    <h3 style="font-size: 16px; margin-bottom: 12px;">⏳ Viaje en el Tiempo (Visual)</h3>
                    <div style="display: flex; gap: 10px; height: 150px;">
                        <div style="flex: 1; position: relative; border-radius: 8px; overflow: hidden; background: #eee;">
                            <div id="pano-now" style="width: 100%; height: 100%;"></div>
                            <span style="position: absolute; bottom: 5px; left: 5px; background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; z-index: 10;">AHORA</span>
                        </div>
                        <div style="flex: 1; position: relative; border-radius: 8px; overflow: hidden; background: #eee;">
                             <div id="pano-before" style="width: 100%; height: 100%; filter: sepia(0.8) grayscale(0.6) contrast(1.2) brightness(0.9);"></div>
                            <span style="position: absolute; bottom: 5px; left: 5px; background: rgba(139, 69, 19, 0.9); color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; z-index: 10;">ANTES</span>
                        </div>
                    </div>
                </div>
                ...
                <!-- ADD TO CHRONODEX BUTTON -->
                 <div class="action-buttons-container" style="margin-bottom: 20px;">
                    <button class="btn-primary" style="width:100%; padding:12px; display:flex; justify-content:center; gap:10px;" onclick="OpenAIService.handleDiscover()">
                        <span>🎯 Descubrir y Guardar</span>
                        <span style="background:rgba(255,255,255,0.2); padding:0 6px; border-radius:10px; font-size:12px;">+${xp} XP</span>
                    </button>
                 </div>
                ...
                <div class="ai-curiosities">
                    <h3 style="font-size: 16px; margin-bottom: 12px; display:flex; align-items:center;">
                        ...
                        <span class="ai-badge" style="background: #667eea; color: white; font-size: 10px; padding: 2px 6px; border-radius: 4px; margin-left:8px;">IA</span>
                    </h3>
                    
                    <div class="curiosities-list">
                        ${aiData.curiosities.map((curiosity, index) => `
                            <div class="curiosity-item" style="display: flex; gap: 12px; margin-bottom:12px; padding: 10px; background: white; border: 1px solid #eee; border-radius: 8px;">
                                <div class="curiosity-number" style="width: 24px; height: 24px; background: ${rarityColor === '#7f8c8d' ? '#667eea' : rarityColor}; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size:12px; font-weight: bold; flex-shrink: 0;">${index + 1}</div>
                                <div class="curiosity-text">
                                    <p style="margin:0; font-size:14px; line-height:1.5;">${curiosity}</p>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                ...
                <!-- QUICK INFO (Specific) -->
                <div class="quick-info" style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-top:20px;">
                     ${aiData.construction_year ? `
                      <div class="info-item" style="background:#f8f9fa; padding:8px; border-radius:6px;">
                ...
                <!-- FOOTER ACTIONS -->
                 <div class="action-buttons" style="margin-top:30px; display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                    <button class="btn-primary" style="background:#4285F4; color:white; border:none; grid-column: 1 / -1; display:flex; justify-content:center; align-items:center; gap:8px; padding:12px; font-weight:bold;" 
                            onclick="OpenAIService.handleStartNavigation()">
                        <span style="font-size:18px;">🚙</span> Cómo llegar
                    </button>
                    
                    <button class="btn-secondary" onclick="OpenAIService.handleShare()">
                        📤 Compartir
                    </button>
                    <button class="btn-secondary" style="background:#f59e0b; color:black; grid-column: 1 / -1; font-weight:bold;" onclick="OpenAIService.handleGeneratePoster()">
                ...
                <div class="ai-disclaimer" style="margin-top: 20px; text-align: center;">
        ...
        setTimeout(() => {
            if (typeof google !== 'undefined' && google.maps) {
                const svOptions = {
                    ...
                    zoom: 1, disableDefaultUI: true, showRoadLabels: false
                };
                new google.maps.StreetViewPanorama(document.getElementById("pano-now"), svOptions);
                new google.maps.StreetViewPanorama(document.getElementById("pano-before"), svOptions);
            }
        }, 500);
    },
