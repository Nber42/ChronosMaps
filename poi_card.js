...
class POIQuickCard {
    ...
    show(basicData) {
        ...
        if (window.map) {
            ...
            window.map.panTo({ lat: offsetLat, lng: basicData.lng });
            if (window.map.getZoom() < 16) window.map.setZoom(16);
        }
    }
    ...
    renderActions(data) {
        ...
        actionsContainer.innerHTML = `
            ...
            <button class="poi-btn poi-btn-secondary" onclick="POIQuickCardInstance.handleDirections(${data.lat}, ${data.lng})">
                <span class="material-icons poi-btn-icon">directions</span>
                <span>Ir</span>
            </button>
        `;
    }
